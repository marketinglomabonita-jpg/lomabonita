...
| **Terreno heredado** | Datos medidos del encargo (Praxis 2.0), verbatim con su metodo de medicion. Opcional — solo modo encargo | Agente al generar |
...
### Criterios de exito
- [ ] [Criterio medible 1 — observable: como se comprueba al mirar o ejecutar (control positivo y negativo)]
- [ ] [Criterio medible 2 — con rango, no igualdad, si es una medicion]
- [ ] [Criterio medible 3 — heredado del encargo o del brief SIN debilitarlo]

> Praxis 2.0: un criterio que nadie puede comprobar mirando o ejecutando no es un criterio —
> investiga como se verifica antes de redactarlo. Quitar o aflojar un criterio heredado es
> re-decision que se anuncia, nunca erosion silenciosa.
...
## Terreno heredado (opcional — modo encargo de Praxis 2.0)

> Voz: impersonal. Solo si el encargo (o el brief que lo cita) trae terreno.
> Si no hay encargo origen: "no hay terreno heredado".

[Dato medido 1 — valor + metodo de medicion + quien lo midio]
[Dato medido 2 — valor + metodo de medicion]

> El terreno es **dato, no conclusion**: viaja verbatim, con su metodo de medicion.
> Tu interpretacion va marcada aparte ("mi lectura de este dato es...") y nunca
> reescribe un numero que otro midio. Si el instrumento que lo midio es dudoso,
> se re-mide ANTES de usar el dato — y se anota que se re-midio.
...
>
> Praxis 2.0: cada `**Validacion**` es observable y no-degradable. Si el
> criterio viene del encargo o del brief, viaja igual o mas fuerte — nunca
> aflojado.

### Fase 1: [Nombre]
- **Objetivo**: [que se logra al completar esta fase]
- **Validacion**: [observable y no-degradable — comando exacto + salida esperada (control positivo) y la señal que prueba que el control no esta roto (control negativo); con rango en vez de igualdad si es medicion; prueba del instrumento si la verificacion depende de una herramienta]

### Fase 2: [Nombre]
- **Objetivo**: [que se logra]
- **Validacion**: [observable y no-degradable — control positivo + negativo]
